...
pub async fn simplex(imgsize: u32, scale: f64, seed: usize) {
    let imgsize = imgsize as usize;
    let open_simplex = OpenSimplexNoise::new(seed.try_into().unwrap());
    ...
    // Apply a Gaussian blur with a sigma of 3.0
    img = blur(&img, 1.0);
